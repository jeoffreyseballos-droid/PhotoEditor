...
Phase 7 adds local adaptive style inference and per-asset recipe generation. Phase 8 adds a separate top-level authoring edge: explicitly selected before/after files are conservatively matched and validated, then feed existing PhotoAnalysis and BatchContext, an isolated staged optimizer derives creative EditRecipe targets, and the exact Phase 7 feature schema feeds a confidence-weighted regularized linear trainer. Dataset/target/run state is persisted in SQLite; valid output is atomically published as a checksummed, versioned Phase 7 package and loaded into the live catalog. Target cache identity depends on source/reference content plus renderer/optimizer/control/mask contracts, never on the downstream trainer version. The React Training Studio only orchestrates typed commands and displays reviews/metrics; decoding, optimization, fitting, package publication and selection mutation stay in Rust. See [TRAINING_STUDIO.md](TRAINING_STUDIO.md).

Generative processing, PhotographerApp API, authentication, cloud, licensing/billing and production signing remain outside this boundary.
